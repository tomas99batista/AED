...
{
  if (n != NULL)    //Faltava este if dai o seg fault
  {
    list_words(n->left);
    printf("%6ld %s\n", n->count, n->word);
    list_words(n->right);
...
int main(int argc, char **argv)
{
  ...
  for (int i = 2; i < argc; i++) //Estava <= mas tem de ser < argc
  {
    // read text file
    ...
    {
    case 'a':
      printf("The file %s contains %ld words\n", argv[i], count_all_words(root));
      break;
    case 'd':
      printf("The file %s contains %ld distinct words\n", argv[i], count_different_words(root));
      break;
    case 'l':
